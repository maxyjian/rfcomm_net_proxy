...
static void rfcomm_client_poll_tap_data(uv_poll_t* handle, int status,
                                        int events)
{
    ...
    if (events & UV_READABLE) {
        if (status == 0) {
            int ret = read(g_client_service.tap_fd, buf, TAP_MAX_PKT_WRITE_LEN);
            if (ret > 0) {
...
static void rfcomm_client_poll_client_data(uv_poll_t* handle, int status,
                                           int events)
{
    uint8_t buf[TAP_MAX_PKT_WRITE_LEN];
    ...
    if (events & UV_READABLE) {
        if (status == 0) {
            int ret
                = read(g_client_service.client_fd, buf, TAP_MAX_PKT_WRITE_LEN);
            if (ret > 0) {
...
static int rfcomm_client_connect(const char* remote_mac)
{
    ...
    g_client_service.client_poll_handle = rfcomm_net_uv_poll_start(
        g_client_service.loop, g_client_service.client_fd,
        UV_DISCONNECT | UV_READABLE, rfcomm_client_poll_client_data, NULL);
    if (g_client_service.client_poll_handle == NULL) {
        ERROR_("poll client fail\n");
        goto fail;
    }

    g_client_service.tap_fd
        = rfcomm_net_tap_open(CLIENT_TAP_IFNAME, bt_mac_str);
    if (g_client_service.tap_fd < 0) {
        ERROR_("open tap fail\n");
        goto fail;
    }

    g_client_service.tap_poll_handle = rfcomm_net_uv_poll_start(
        g_client_service.loop, g_client_service.tap_fd,
        UV_DISCONNECT | UV_READABLE, rfcomm_client_poll_tap_data, NULL);
